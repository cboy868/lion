 ...
 */
class GoodsAvRel extends ActiveRecord
{
    ...
    {
        return $this->hasOne(GoodsAttrVal::className(), ['id' => 'av_id'])->where(['status'=>GoodsAttrVal::STATUS_ACTIVE]);
    }
    ...
    public static function parsePost($category_id, $goods_id)
    {
        ...
        foreach ($data as $k => $v) {
            $attr = GoodsAttr::findOne($k);

            if (is_array($v)) {
                foreach ($v as $value) {
                    $nv = '';
                    $av_id = $value;
                    if ($attr->is_multi == GoodsAttr::MULTI_SELF) {
                        $nv =$value;
                        $av_id = 0;
                    }
    ...
    public static function getAttrsAndSpecs($category_id=null, $filters=[])
    {
        $models = self::find()->filterWhere(['category_id'=>$category_id])
                              ->andFilterWhere(['not in', 'attr_id', $filters]);
        $list = $models->all();
    ...
    public static function attrs($category_id=null, $filters=[])
    {
        ...
        $result = self::getAttrsAndSpecs($category_id=null, $filters=[]);

        $attr_ids = array_keys($result);

        $atrs = GoodsAttr::find()
                    ->where(['id'=>$attr_ids, 'is_spec'=>1, 'status'=>GoodsAttr::STATUS_ACTIVE])
                    ->asArray()
                    ->all();
    ...
    public static function specs($category_id=null, $filters=[])
    {
        ...
        $atrs = GoodsAttr::find()
                    ->where(['id'=>$attr_ids, 'is_spec'=>0, 'status'=>GoodsAttr::STATUS_ACTIVE])
                    ->asArray()
                    ->all();
