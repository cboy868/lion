...
<div id="agency-year-persent" style="height:400px;"></div>
...
function getYearPercent(year) {
    var myChart = echarts.init(document.getElementById('agency-year-persent'), 'vintage');
    $.get('<?=Url::toRoute('/analysis/admin/agency/year')?>?year='+year).done(function (data) {
        var data = data.data;
        var result = [];
        for (var i in data){
            result.push({
