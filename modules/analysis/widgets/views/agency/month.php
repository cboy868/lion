...
<div>
    <h4 style="text-align: right">
        月份选择
        <?=\yii\helpers\Html::dropDownList('month',
            null,
            [0=>'全年'] + months(),
            ['class'=>'selMonthCompare'])?>
    </h4>
</div>
    <div id="agency-month-compare" style="height:400px;"></div>
...
$(function(){
    ...
    function getMonthCompare(month) {
        ...
        var myChart = echarts.init(document.getElementById('agency-month-compare'), 'vintage');
        $.get('<?=Url::toRoute('/analysis/admin/agency/month')?>?month='+month).done(function (data) {
            var data = data.data;
            var name = [],value=[];
            for (var i in data){
