...
<div class="album-form">
    ...
    <?php foreach ($attach as $k => $v): ?>
        <?php if ($v['html'] == 'input'): ?>
            <?= $form->field($model, $v['name'])
                ->textInput(['value'=>isset($model)?$model->$v['name']:''])
                ->hint($v['pop_note']) ?>
        ...
        <?php elseif($v['html'] == 'fulltext'): ?>
            <?= $form->field($model,$v['name'])
                ->widget('app\core\widgets\Ueditor\Ueditor',['option' =>['res_name'=>'album', 'use'=>'ue'], 'value'=>isset($model)?$model->$v['name']:''])
                ->label($v['title'])
                ->hint($v['pop_note']); ?>
        <?php endif ?>
    <?php endforeach ?>
